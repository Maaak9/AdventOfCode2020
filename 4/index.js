...
// Fix data so that we have an array of objects with passport data with key:value
const passports = puzzleData.split('\n\n').map((passport) => {
  return passport.split(/\↵| |\n/).reduce((accumulator, passportData) => {
    const keyAndValue = passportData.split(':');
    return { ...accumulator, ...{ [keyAndValue[0]]: keyAndValue[1]} };
  }, {});
})

const checkIfPassportIsValid = (passport) => {
  const requiredFields = [ "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" ];
  let isPassportValid = true;
  requiredFields.forEach((requiredField) => {
    if (passport[requiredField] === undefined) isPassportValid = false;
  })
...
passports.forEach((passport) => {
  if (checkIfPassportIsValid(passport)) validPassports++;
})

console.warn(`The number of valid passports: ${validPassports}`);
...
const checkIfPassportIsValidPart2 = (passport) => {
  const requiredFields = [ "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" ];
  for (let i = 0; i < requiredFields.length; i++) {
    let isPassportValid = false;
    const currentField = passport[requiredFields[i]];
    if (currentField !== undefined) {
      switch (requiredFields[i]) {
        case "byr":
          if (parseInt(currentField) >= 1920 && parseInt(currentField) <= 2002) {
            isPassportValid = true;
          }
            break;
        case "iyr":
          if (parseInt(currentField) >= 2010 && parseInt(currentField) <= 2020) {
            isPassportValid = true;
          }
          break;
        case "eyr":
          if (parseInt(currentField) >= 2020 && parseInt(currentField) <= 2030) {
            isPassportValid = true;
          }
          break;
        case "hgt":
          if ((/cm/.test(currentField) && (parseInt(currentField) >= 150 && parseInt(currentField) <= 193)) ||
              (/in/.test(currentField) && (parseInt(currentField) >= 59 && parseInt(currentField) <= 76))) {
                isPassportValid = true;
          }
          break;
        case "hcl":
          if (/^#[0-9a-f]{6}$/.test(currentField)) {
            isPassportValid = true;
          }
          break;
        case "ecl":
          if (currentField === "amb" || currentField === "blu" ||
              currentField === "brn" || currentField === "oth" ||
              currentField === "gry" || currentField === "grn" ||
              currentField === "hzl") {
                isPassportValid = true;
              }
          break;
        case "pid":
          if (currentField.length !== 9) break;
          else {
            if (/^[0-9]{9}$/.test(currentField)) {
              isPassportValid = true;
             }
          }
          break;
...
console.warn('1', checkIfPassportIsValidPart2({
  pid: "087499704",
  hgt: "74in",
  ecl: "grn",
  iyr: "2012",
...
console.warn('2', checkIfPassportIsValidPart2({
  pid: "186cm",
  hgt: "180",
  ecl: "amb",
  iyr: "2018",
...
passports.forEach((passport) => {
  if (checkIfPassportIsValidPart2(passport)) validPassports2++;
})

console.warn(`Part2: The number of valid passports: ${validPassports2}`);