...
for (let i = 0; i < expenseReport.length; i++) {
  for (let y = 1; y < expenseReport.length; y++) {
    for (let x = 3; x < expenseReport.length; x++) {
      if ((expenseReport[i] + expenseReport[y] + expenseReport[x]) === 2020) {
        console.warn(`${expenseReport[i]} * ${expenseReport[y]} * ${expenseReport[x]} = `, expenseReport[i] * expenseReport[y] * expenseReport[x]);
        y = 100000; i = 100000; x = 100000;
      }
    }
  }
}
